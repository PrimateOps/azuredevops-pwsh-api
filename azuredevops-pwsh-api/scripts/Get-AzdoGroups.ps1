<#
.SYNOPSIS
Lists all Groups in an Organisation.

.DESCRIPTION
Lists all Groups in an Organisation. Subsequent filtering on the return object is then possible.

.NOTES
API Docs: https://docs.microsoft.com/en-us/rest/api/azure/devops/graph/groups/list?view=azure-devops-rest-6.0&tabs=HTTP
...
param(
    ...
    try{
        $apiUri = (Read-AzdoApiList -ErrorAction Stop | Select-Object -ExpandProperty groups).list
    }catch{
        $ex = $_
